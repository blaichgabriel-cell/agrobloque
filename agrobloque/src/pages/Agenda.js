...
const TIPOS = {
  ...
  cosecha: { label:'Cosecha', icon:'ti-cut', color:'#2d6a2d', bg:'#edf7ed' },
...
export default function Agenda() {
  ...
  const getBadge = (t) => {
    if (t.completada) return { label:'Completada', bg:'#edf7ed', color:'#2d6a2d' }
            ...
            <div key={t.id} style={{ background:'#fff', borderRadius:20, padding:'14px 16px', marginBottom:8, opacity: t.completada ? 0.6 : 1 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:10, borderTop:'1px solid #f2f1ef' }}>
                ...
                <div style={{ display:'flex', gap:6 }}>
                  {!t.completada && (
                    <button onClick={() => completarTarea(t.id)} style={{ padding:'5px 12px', borderRadius:10, border:'1px solid #e8e6e2', background:'transparent', fontSize:11, fontWeight:500, color:'#2d6a2d', cursor:'pointer' }}>
                      ✓ Completar
                    </button>
                  )}
      ...
      {modal && (
        <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.4)', zIndex:100, display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={e => e.target===e.currentTarget && setModal(false)}>
          <div style={{ background:'#f2f1ef', borderRadius:'24px 24px 0 0', width:'100%', maxWidth:480, padding:'24px 20px 40px', maxHeight:'85vh', overflowY:'auto' }}>
            ...
            <button style={{ width:'100%', padding:14, borderRadius:14, background:'#0a0a0a', border:'none', fontSize:14, fontWeight:700, color:'#fff', cursor:'pointer' }} onClick={guardarTarea} disabled={saving}>
