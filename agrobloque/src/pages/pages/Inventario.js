...
export default function Inventario() {
  ...
  const [form, setForm] = useState({ nombre:'', categoria_id:'', principio_activo:'', unidad:'kg', stock_actual:'', stock_minimo:'', notas:'' })
  ...
  const guardar = async () => {
    ...
    setSaving(true)
    if (form.id) {
      await supabase.from('productos').update({ nombre:form.nombre, categoria_id:form.categoria_id||null, principio_activo:form.principio_activo||null, unidad:form.unidad, stock_actual:Number(form.stock_actual)||0, stock_minimo:Number(form.stock_minimo)||0, notas:form.notas||null }).eq('id', form.id)
    } else {
      await supabase.from('productos').insert({ nombre:form.nombre, categoria_id:form.categoria_id||null, principio_activo:form.principio_activo||null, unidad:form.unidad, stock_actual:Number(form.stock_actual)||0, stock_minimo:Number(form.stock_minimo)||0, notas:form.notas||null })
    }
    await fetchProductos(); setSaving(false); setModal(null)
    setForm({ nombre:'', categoria_id:'', principio_activo:'', unidad:'kg', stock_actual:'', stock_minimo:'', notas:'' })
  }

  const ajustarStock = async (id, delta) => {
    const p = productos.find(x => x.id === id)
    if (!p) return
    const nuevo = Math.max(0, Number(p.stock_actual) + delta)
    await supabase.from('productos').update({ stock_actual: nuevo }).eq('id', id)
    fetchProductos()
  }
  ...
  const getStockColor = (p) => {
    if (p.stock_actual <= 0) return '#c84040'
    if (p.stock_actual <= p.stock_minimo) return '#e07b00'
    return '#2d6a2d'
  }

  const getStockBg = (p) => {
    if (p.stock_actual <= 0) return '#fff0f0'
    if (p.stock_actual <= p.stock_minimo) return '#fff3e8'
    return '#edf7ed'
  }

  const productosFiltrados = filtro === 'todos' ? productos : filtro === 'bajo' ? productos.filter(p => p.stock_actual <= p.stock_minimo) : productos.filter(p => p.categorias_producto?.nombre === filtro)
    ...
    <div style={{ background:'#f2f1ef', minHeight:'100vh' }}>
      <div style={{ background:'#f2f1ef', padding:'24px 20px 16px' }}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:16 }}>
          ...
          <button onClick={() => setModal('nuevo')} style={{ width:40, height:40, borderRadius:14, background:'#0a0a0a', border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', marginTop:2 }}>
        ...
        <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:4 }}>
          {['todos','bajo',...CATEGORIAS].map(f => (
            <button key={f} onClick={() => setFiltro(f)} style={{ padding:'7px 14px', borderRadius:20, border:'none', fontSize:11, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap', background: filtro===f ? '#0a0a0a' : '#e8e6e2', color: filtro===f ? '#fff' : '#9a9a9a' }}>
      ...
      <div style={{ padding:'12px 14px 100px' }}>
        {productosFiltrados.length === 0 ? (
          ...
          <div key={p.id} style={{ background:'#fff', borderRadius:20, padding:'14px 16px', marginBottom:8 }}>
            ...
            <div style={{ background: getStockBg(p), borderRadius:8, padding:'4px 10px', marginBottom:10, display:'inline-block' }}>
              <div style={{ fontSize:10, fontWeight:600, color: getStockColor(p) }}>
                {p.stock_actual <= 0 ? 'Sin stock' : p.stock_actual <= p.stock_minimo ? 'Stock bajo · mínimo: ' + p.stock_minimo + ' ' + p.unidad : 'Stock OK · mínimo: ' + p.stock_minimo + ' ' + p.unidad}
              </div>
            </div>
            <div style={{ display:'flex', gap:6, alignItems:'center' }}>
              ...
              <button onClick={() => ajustarStock(p.id, 1)} style={{ width:32, height:32, borderRadius:10, border:'1px solid #e8e6e2', background:'#f2f1ef', fontSize:18, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'#555' }}>+</button>
              <button onClick={() => { setForm({...p, categoria_id: p.categoria_id||''}); setModal('nuevo') }} style={{ padding:'5px 12px', borderRadius:10, border:'1px solid #e8e6e2', background:'transparent', fontSize:11, color:'#555', cursor:'pointer', marginLeft:4 }}>Editar</button>
              <button onClick={() => eliminar(p.id)} style={{ padding:'5px 12px', borderRadius:10, border:'1px solid #ffcccc', background:'transparent', fontSize:11, color:'#c84040', cursor:'pointer' }}>Eliminar</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {modal && (
        <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.4)', zIndex:100, display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={e => e.target===e.currentTarget && setModal(null)}>
          <div style={{ background:'#f2f1ef', borderRadius:'24px 24px 0 0', width:'100%', maxWidth:480, padding:'24px 20px 40px', maxHeight:'85vh', overflowY:'auto' }}>
            ...
            {[['Nombre *','nombre','text','Ej: Fungicida Captan'],['Principio activo','principio_activo','text','Ej: Captan 80%'],['Stock actual','stock_actual','number','0'],['Stock mínimo (alerta)','stock_minimo','number','0'],['Notas','notas','text','Opcional']].map(([lbl,key,type,ph]) => (
              <div key={key}>
                <div style={{ fontSize:10, color:'#9a9a9a', marginBottom:6 }}>{lbl}</div>
                <input style={{ width:'100%', padding:'11px 14px', borderRadius:12, border:'1px solid #e8e6e2', background:'#fff', fontSize:13, color:'#0a0a0a', marginBottom:12 }} type={type} value={form[key]||''} onChange={e => setForm(f => ({...f,[key]:e.target.value}))} placeholder={ph}/>
              </div>
            ))}
            <div style={{ fontSize:10, color:'#9a9a9a', marginBottom:6 }}>Categoría</div>
            <select style={{ width:'100%', padding:'11px 14px', borderRadius:12, border:'1px solid #e8e6e2', background:'#fff', fontSize:13, color:'#0a0a0a', marginBottom:12 }} value={form.categoria_id||''} onChange={e => setForm(f => ({...f, categoria_id:e.target.value}))}>
