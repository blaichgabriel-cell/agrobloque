...
export default function Compradores() {
  ...
  const inpStyle = { width:'100%', padding:'11px 14px', borderRadius:8, border:"1px solid #e2e9e5", background:'#fff', fontSize:13, color:"#182c25", marginBottom:12 }
  const lblStyle = { fontSize:10, color:"#697970", marginBottom:6, display:'block' }
    ...
    <div style={{ background:"#f6f8f7", minHeight:'100vh' }}>
      ...
      <div style={{ background:"#f6f8f7", padding: isDesktop ? '34px 36px 18px' : '24px 20px 16px' }}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:4 }}>
          <div>
            ...
            <div style={{ fontSize:24, fontWeight:700, color:"#182c25", letterSpacing:-.5 }}>Compradores</div>
          </div>
          <button onClick={() => { setForm({ nombre:'', tipo:'Mayorista', telefono:'', notas:'' }); setModal(true) }} style={{ width:40, height:40, borderRadius:8, background:"#124e38", border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
      ...
      <div style={{ padding: isDesktop ? '8px 36px 100px' : '8px 14px 100px' }}>
        {compradores.length === 0 ? (
          ...
          return (
            <div key={c.id} style={{ background:'#fff', borderRadius:8, padding:'14px 16px', marginBottom: isDesktop ? 0 : 8, boxShadow: isDesktop ? '0 10px 28px rgba(29,38,29,0.045)' : 'none' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }} onClick={() => setExpandido(isOpen ? null : c.id)}>
                <div style={{ width:40, height:40, borderRadius:8, background:'#eeeeee', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:"#697970", marginTop:1 }}>{c.tipo}{c.telefono ? ' · ' + c.telefono : ''}</div>
                </div>
                <div style={{ textAlign:'right' }}>
                  {stats.operaciones > 0 ? (
                    <>
                      <div style={{ fontSize:13, fontWeight:700, color:"#124e38" }}>{stats.kg.toLocaleString()} kg</div>
                      <div style={{ fontSize:10, color:"#697970" }}>{stats.operaciones} operaciones</div>
                    </>
                  ) : (
                ...
                <div style={{ marginTop:12, paddingTop:12, borderTop:"1px solid #f6f8f7" }}>
                  {stats.operaciones > 0 && (
                    <div style={{ display:'flex', gap:8, marginBottom:12 }}>
                      <div style={{ flex:1, background:"#f6f8f7", borderRadius:8, padding:'10px 12px', textAlign:'center' }}>
                        <div style={{ fontSize:9, color:"#697970", marginBottom:3 }}>Total kg</div>
                        <div style={{ fontSize:14, fontWeight:700, color:"#182c25" }}>{stats.kg.toLocaleString()}</div>
                      </div>
                      <div style={{ flex:1, background:'#eeeeee', borderRadius:8, padding:'10px 12px', textAlign:'center' }}>
                        <div style={{ fontSize:9, color:"#697970", marginBottom:3 }}>Precio prom</div>
                        <div style={{ fontSize:14, fontWeight:700, color:"#124e38" }}>Gs. {stats.precioProm.toLocaleString()}</div>
                      </div>
                      <div style={{ flex:1, background:"#f6f8f7", borderRadius:8, padding:'10px 12px', textAlign:'center' }}>
                        <div style={{ fontSize:9, color:"#697970", marginBottom:3 }}>Ingresos</div>
                  ...
                  {(historial[c.id] || []).slice(0, 4).map((h, i) => {
                    return (
                      <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', borderBottom:"1px solid #f6f8f7" }}>
                        <div>
                          <div style={{ fontSize:11, color:"#182c25" }}>{h.producto || 'Venta'}{h.bloques?.codigo ? ' - Bloque ' + h.bloques.codigo : ''} · {h.kg_total} kg</div>
                          <div style={{ fontSize:10, color:"#697970" }}>{h.fecha}</div>
                        </div>
                        {h.precio_kg > 0 && <div style={{ fontSize:11, fontWeight:600, color:"#124e38" }}>Gs. {Number(h.precio_kg).toLocaleString()}/kg</div>}
                      </div>
                    )
                  })}

                  <div style={{ display:'flex', gap:6, marginTop:12 }}>
                    <button onClick={() => { setForm({...c}); setModal(true) }} style={{ flex:1, padding:'8px', borderRadius:8, border:"1px solid #e2e9e5", background:'transparent', fontSize:12, color:'#555', cursor:'pointer' }}>Editar</button>
                    <button onClick={() => eliminar(c)} style={{ flex:1, padding:'8px', borderRadius:8, border:'1px solid #ffcccc', background:'transparent', fontSize:12, color:'#c84040', cursor:'pointer' }}>Eliminar</button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.4)', zIndex:100, display:'flex', alignItems: typeof window !== 'undefined' && window.innerWidth >= 768 ? 'center' : 'flex-end', justifyContent:'center' }}>
          <div style={{ background:"#f6f8f7", borderRadius: typeof window !== 'undefined' && window.innerWidth >= 768 ? 24 : '24px 24px 0 0', width:'100%', maxWidth:480, padding:'24px 20px 40px', maxHeight:'80vh', overflowY:'auto', boxShadow: typeof window !== 'undefined' && window.innerWidth >= 768 ? '0 24px 70px rgba(0,0,0,0.24)' : 'none' }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:12 }}>
              {TIPOS.map(t => (
                <button key={t} onClick={() => setForm(f => ({...f, tipo:t}))} style={{ padding:'8px 14px', borderRadius:8, border:"1px solid #e2e9e5", fontSize:12, fontWeight:500, cursor:'pointer', background: form.tipo===t ? '#212121' : '#fff', color: form.tipo===t ? '#fff' : '#555' }}>{t}</button>
              ))}
            </div>
