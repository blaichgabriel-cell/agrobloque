...
export default function Overview({ campoActivo, setCampoActivo, isGuest = false, role }) {
  ...
  return <div className="ag-overview">
    ...
    <section className="ag-hero"><div><span className="ag-eyebrow">AgroBloque · {campoActivo?.nombre || 'Tu campo'}</span><h1>{nombre ? `Hola, ${nombre}` : 'Buen día'}</h1><p>Un campo bien gestionado,<br />siempre da buenos frutos.</p></div><div className="ag-hero-actions"><span className="ag-hero-caption">El campo en control</span>{mayWork && <button className="ag-primary" onClick={() => setWork(true)}><i className="ti ti-plus" />Registrar trabajo</button>}</div></section>
    ...
    <div className="ag-metrics">
      {can('cosecha') && <Metric icon="ti-leaf" label="Producción del mes" value={value('cosechas', monthlyKg > 0 ? `${number(monthlyKg)} kg` : 'Sin cosechas este mes')} detail={new Date(`${today}T12:00:00`).toLocaleDateString('es-PY', { month: 'long', year: 'numeric' })} compact={monthlyKg <= 0} onClick={() => navigate('/cosecha')} />}
      <Metric icon="ti-layout-grid" label="Bloques activos" value={value('bloques', active)} detail={`de ${value('bloques', data.bloques.length)} bloques`} onClick={() => navigate('/mapa')} />
      {!isGuest && can('asistencia') ? <Metric icon="ti-users" label="Personal registrado" value={value('operarios', data.operarios.length)} detail="Operarios del campo" onClick={() => navigate('/asistencia')} /> : <Metric icon="ti-seeding" label="Plantaciones activas" value={value('plantas', data.plantas.length)} detail="En el campo seleccionado" onClick={() => navigate('/mapa')} />}
      {can('agenda') && <Metric icon="ti-clipboard-list" label="Tareas pendientes" value={value('tareas', data.tareas.length)} detail="Agenda del campo" warning={data.tareas.length > 0} onClick={() => navigate('/agenda')} />}
    </div>
