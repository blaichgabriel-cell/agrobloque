-- AgroBloque: fix v3 para invitados.
-- Ejecutar en Supabase SQL Editor.
-- No usa pgcrypto, gen_random_bytes ni digest.
-- Crea links nuevos con token seguro basado en random + tiempo + transaccion.

alter table if exists public.guest_access_links
  add column if not exists permisos jsonb;

create or replace function public.create_guest_access_link_v3(
  p_nombre text,
...
begin
  ...
  raw_token :=
    ...
    md5(random()::text || clock_timestamp()::text || p_nombre);
...
grant execute on function public.create_guest_access_link_v3(text, text, text, text) to authenticated;
