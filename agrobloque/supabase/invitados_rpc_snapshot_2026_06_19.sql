-- AgroBloque: solucion estable para links de invitado.
-- Esta version NO depende del header x-guest-token.
-- El invitado lee un resumen por RPC usando el token del link como parametro.
-- No habilita crear, editar ni borrar datos.
    ...
    'plantaciones', coalesce((
      ...
      from (
        select
          ...
          p.fecha_siembra,
          p.densidad_plantas_m2 as cantidad_plantas
        from public.plantaciones p
        join public.bloques b on b.id = p.bloque_id
