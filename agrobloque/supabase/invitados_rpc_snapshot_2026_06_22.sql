-- AgroBloque: solucion final para links de invitado.
-- Ejecutar en Supabase SQL Editor.
-- Supabase genera el token y tambien lo valida, evitando diferencias entre navegador y base de datos.
-- No habilita crear, editar ni borrar datos para invitados.
...
create or replace function public.create_guest_access_link(
  p_nombre text,
...
begin
  ...
  if p_dias is null or p_dias <= 0 then
    ...
    vencimiento := now() + make_interval(days => p_dias);
  end if;
...
grant execute on function public.create_guest_access_link(text, uuid, integer, jsonb) to authenticated;
